...
int query(int l, int r){
    ...
    for(int k = 25; k >= 0; k--)
        if(depth[ncstr[k][r]] >= depth[l])
            res = max(res, minw[k][r]),
            r = ncstr[k][r];
    if(l == r) return res;
    for(int k = 25; k >= 0; k--){
        if(ncstr[k][l] != ncstr[k][r])
            res = max(res, max(minw[k][l], minw[k][r])),
            l = ncstr[k][l], r = ncstr[k][r];
    }
    res = max(res, max(minw[0][l], minw[0][r]));
    return res;
}
int main(){
    n = read(), m = read(), q = read();
    memset(minw, 0x80, sizeof(minw));
    //cerr << minw[1][1] << endl;
    for(int i = 1; i <= m; i++){
    ...
    for(int j = 1; j <= 25; j++)
        for(int i = 1; i <= n; i++)
            ...
            minw[j][i] = max(minw[j-1][i], minw[j-1][ncstr[j-1][i]]);
    while(q--){
        x = read(), y = read();
        printf("%d\n", query(x, y));
    }
